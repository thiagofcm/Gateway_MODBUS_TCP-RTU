...
/* ----------------------- Defines  -----------------------------------------*/
//define se a porta modbus será pela USB ou pela porta ethernet da BBB : 0=A porta sera a Eth0 ; 1=A porta vai ter IP Fixo 192.168.7.2
#define PORTA_Modbus_USB    0
...
/* Quando na BBB, define se a porta de comunicacao modbus será a USB (192.168.7.2) ou a Ethernet0 (que o ip depende da conexao externa)) 
*/
#if (PORTA_Modbus_USB == 1)
    serveraddr.sin_addr.s_addr = inet_addr("192.168.7.2");   // Se for compilar para a BBB#
    StatusModbusPort = 1;
#else
    serveraddr.sin_addr.s_addr = htonl( INADDR_ANY );        // Se for compilar para o PC
    StatusModbusPort = 2;
#endif
